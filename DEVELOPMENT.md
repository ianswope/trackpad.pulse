...
- **Telemetry** is the recorder in `collectors/trackpad_pulse.py`. `Pad` is pure: feed it events, read counters. `Recorder` owns the files. The panel reads `snapshot.json`, `history.json` and the tmpfs `live.json` through `FileView` and never writes them. `Pulse.js` formats and derives readouts; `TrackpadChip.qml`, `TouchHistoryGraph.qml` and `SpeedHistogram.qml` draw.
